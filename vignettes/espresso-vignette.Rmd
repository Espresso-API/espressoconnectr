---
title: "espresso-vignette"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{espresso-vignette}
...
Espresso API is a collection of HTTPS-REST APIs that provide integration with our in-house trading
...
The workflow starts with the creation of a espresso connect object and completing a successful login flow. 
To create a espresso connect object the following may be used. 
...
(https://api.myespresso.com/espressoapi/auth/login.html) for that `api_key`.

## API Handshake

Once the `request_token` is available, the `set_request_token` function can be used to update the espresso connect object and then call the `fetch_access_token` to complete the API handshake and obtain an `access_token`. A successful handshake will set the `access_token` slot inside the espresso connect object and return the object itself. From this point onward the handshake is complete and the API functions are available to the user.
...
report_order_response <- get_report_history(connect, report_method_params) (Order History)
...
master_response <- get_master(connect, master_params) (With Authorization)
master_csv_response <- get_master_csv(connect, master_csv_params) (Without Authorization)
