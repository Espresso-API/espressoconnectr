...
is_connection_object <- function(object){
  return(class(object) == "espressoconnect")
}
...
#'Function to set the request token in the espresso connect object.
#'@description Function to set request token (obtained from a successful
#'login flow) in the espresso connect object.
#'@param object an S4 object of type espressoconnect.
#'@param request_token a string containing the request token from login flow.
#'@details  This function allows to set the request token of the espresso
...
#'@seealso \code{\link[espressoconnect]{create_connection_object}}
#'@export
set_request_token <- function(object,request_token){
  if(!is_connection_object(object)){
    message("Invalid espresso connect object")
  }
...
#'Function to set the access token in the espresso connect object.
#'@description This function sets the access token (obtained by sending a
#'hash of api key, request token and api secret) inside the espresso connect
#'object.
#'@param object a espresso connect object.
#'@param access_token Access token.
#'@return espresso connect object with access token set.
#'@seealso \code{\link[espressoconnect]{create_connection_object}}
#'@seealso \code{\link[espressoconnect]{fetch_access_token}}
#'@export
...
#'the espresso object.
#'@param object An object of espressoconnect type
#'@param vendor_key  A vendor_key with default value NULL
#'@return Returns the login URL.
#'@export
get_login_url <- function(object, vendor_key=NULL){
  if(!is_valid_connection(object)){
    message("Invalid espresso connect object")
    return(NULL)
  }

  if(!is.null(vendor_key)) {
    return(paste0(object@login,"?&api_key=",object@api_key,"&vendor_key=",vendor_key,"&state=12345"))
  } else {
    print("Vendor key not provided")
  }

  return(paste0(object@login,"?&api_key=",object@api_key,"&state=12345"))
}
...
#'Create a espresso connect connection object.
#'@description This function returns an object of espressoconnect type, given the
#'input parametrs.
...
#'the developer page in your espresso app, and must be supplied during object
...
#'@return S4 object for espressoconnect which can be used in further api calls.
#'@export
create_connection_object <- function(params){
  object = methods::new("espressoconnect")
  ...
  if(!is_valid_connection(object)){
    message("Failed to create espresso connect object")
    return(NULL)
  }
  return(object)
}
...
#'@param object An object of type espresso connect, must have request token set.
...
#'@seealso \code{\link[espressoconnect]{set_request_token}}
#'@export
fetch_access_token <- function(object){
  access_token <- NULL
  if(!is_logged_connection(object)){
    message("Invalid espresso connect object or request token not set")
    return(NULL)
  }
