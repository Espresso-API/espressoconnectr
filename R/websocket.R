...
espresso_connect_object<-function(params){
  object = methods::new("espressowebSocket")
  tryCatch({
    ...
    object@ws_url = ifelse(is.null(params[["ws_url"]]),api_websoket,
                           params[["ws_url"]])
...
espressoSocket.connect<-(function(object){
  url<-paste(object@ws_url,"?ACCESS_TOKEN=",object@access_token,"&API_KEY=",object@api_key,sep = "")
  message(url)
