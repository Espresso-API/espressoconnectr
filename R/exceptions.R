...
EspressoException <- condition(c("EspressoException", "error"),
...
espresso_error_msg = "Espresso: Error"
is_espresso_error <- function(x){
  return(length(grep(espresso_error_msg,x))>0)
}
