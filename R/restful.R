...
rest_api_call <- function(object,method,endpoint,route_params,method_params){
  ...
  if(length(r$data)>0){
    if(is_espresso_error(toString(r$data[[1]]))){
      stop(espressoException)
    }
  }
...
#'@param object An object of type espresso connect with valid api_key and
#'access_token.
...
get_funds <- function(object, param){
  ...
  tryCatch({r <- rest_api_call(object,"GET", "orders.fund", route_params, method_params)
    ...
    if(is.null(r)) {
      message("Empty body in response or other exception")
    } else {
      message(e$message)
    }
  })
...
place_order <- function(object, param){
  ...
  tryCatch({r <- rest_api_call(object,"POST","orders.place", route_params, method_params)
    ...
    if(is.null(r)) {
      message("Empty body in response")
    } else {
      message(e$message)
    }
  })
...
#'@description Function to cancel an existing order by Id.
#'@param object An object of type espresso connect with valid api_key and
#'access_token.
#'@param param This contains the request parameters.
#'@return Returns a json object with order details, if successful.
#'@export
cancel_order_by_id <- function(object, param){
  r <- NULL
  route_params <- list("orderid" = param$orderid)
  method_params <- list()
  tryCatch({r <- rest_api_call(object,"GET", "orders.cancelId", route_params, method_params)
  }, error=function(e){
    if(is.null(r)) {
      message("Empty body in response")
    } else {
...
#'Function to get the latest symbol of exchanges.
#'@description Gets all the latest symbols of all the exchanges from the symbol
#'master files as CSV without access-token authorization.
#'@param object An object of type espresso connect with valid api_key and
#'access_token.
#'@param param This contains the request parameters.
#'@return Returns a json object with exchnage symbol details, if successful.
#'@export
get_master_csv <- function(object, param) {
  url <- paste0(object@root, object@routes[["orders.symbol"]], param$exchange)
  response <- httr::GET(url)
  if (httr::status_code(response) != 200) {
    stop("HTTP error ", httr::status_code(response))
  }
  # r <- read.csv(text = httr::content(response, as = "text"))
