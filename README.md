...
``` r
Once you have defined the necessary parameters in the 'params' list, you can pass them to the 'create_connection_object()' function to create a new connection object. The function will return an object of the 'espressoconnect' type, which can then be assigned to a variable named connect.
connect <- create_connection_object(params = params)
```

# make a call to get_login_url print the login url
``` r
The 'get_login_url()' function requires two parameters: an object of type 'espressoconnect' and a 'vendor_key' (which has a default value of NULL). 
To call the function and generate a login URL, you should first pass the espressoconnect object (connect) to the function as follows: 
...
We will pass the espressoconnect type object to 'fetch_access_token' in which we have encryption-decryption logic and by successful run it will return
a response which contain out access token.Encryption decryption we used reticulate library which is Reticulate embeds a Python session within your R session, enabling seamless, high-performance interoperability i.e. we can use Python and R together(You also need Python installed in your system).
```

# place order history
```r
new_order_method_params={ "customerId": XXXXXX, "scripCode": 2475, "tradingSymbol": "ONGC", "exchange": "NC", "transactionType": "B", "quantity": 1, "disclosedQty": 0, "price": "145.85", "triggerPrice": "0", "rmsCode": "ANY", "afterHour": "N", "orderType": "NORMAL", "channelUser": "XXXXXX", "validity": "GFD", "requestType": "NEW", "productType": "CNC" }
...
params which contains other necessary parameters also to 'socketObject <- espresso_connect_object(params)'
which return espressowebSocket type object in response and then we send it to
'espressoSocket.connect(socketObject)' and make a call.
...
including the values of action, key, and value from the espressowebSocket object.
